#!/usr/bin/env python3
"""
pre-init — 在指定目录初始化一个 claude agent (幂等).

用法:
  pre-init [target_dir] [--mode ...] [--tmux-session ...] [...]

默认 target_dir = $(pwd). 建议在 agent 实际目录主动执行;
把 <pre-repo>/scripts/pre_init.py 软链到 ~/.local/bin/pre-init 入 PATH.

输出 InitResult: 创建 / 跳过 / 冲突 / 下一步.
exit 0 = ok, 1 = 有 conflict / failure / tmux 未起.

行为:
- 配置真源 = target_dir/pre/agent_config.json (规则也在这里).
- pre_rule/agents/<dir>/agent_pointer.json 只是 driver 索引指针 (含 cwd).
- 已有 hook 冲突 → 报 conflicts, 不强改.
- 已有 pointer cwd 不一致 → 报 conflicts.
- tmux 没起 → next_steps 提示用户退 claude + bash spawn_agent.sh.
"""
import argparse
import asyncio
import os
...
async def _run(target_dir: str, opts: dict):
    driver = CliClaudeCodeLocalDriver()
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Initialize a claude agent in target_dir (idempotent).",
    ...
    result = asyncio.run(_run(target_dir, opts))
